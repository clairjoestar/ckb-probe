...
```bash
ckb-probe symbols /path/to/ckb --tier 1   # 只看 Tier 1（RocksDB C API 符号）
ckb-probe symbols /path/to/ckb --tier 2   # 只看 Tier 2（Rust 函数符号）
ckb-probe symbols /path/to/ckb --tier 3   # 只看 Tier 3（不可用的符号）
```
